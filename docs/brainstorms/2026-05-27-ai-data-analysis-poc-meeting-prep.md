...
| **2** | **Wren AI `main`** ([Canner/WrenAI](https://github.com/Canner/WrenAI)) | OSS **context layer** (MDL + planner + memory) for agents — CLI/SDK/`wren-langchain`; **not** the archived GenBI Docker UI (`legacy/v1`) | Git-native semantics, Bedrock-friendly (you bring LLM), composes with Deep Agents; inspectable `dry-plan` | MDL upfront; you run orchestration; duplicate semantics if Snowflake Semantic Views also exist |
| **2b** | **Snowflake Cortex Analyst** ([docs](https://docs.snowflake.com/en/user-guide/snowflake-cortex/cortex-analyst)) | Warehouse-native NL→SQL over **Semantic Views** + REST API | Native RBAC/governance, managed text-to-SQL, no Wren ops | Snowflake LLMs only (not Bedrock); Snowflake-only; per-message Analyst credits |
